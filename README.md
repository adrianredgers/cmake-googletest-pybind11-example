...
- CMake target `myfactorial` compiles `PyModule.cpp` links with `FactorialLib` and `pybind11` to build the Python module (shared library) `myfactorial.so`
...
- An **out-of-source** build creates build artifacts in a directory (structure) that you choose.
    - Easy to remove and re-generate all build artifacts: just delete the out-of-source build dir.
...
**CLion tips:**
- [Don't understand why you can't edit any files? CLion uses VIM by default. Get rid of VIM by removing the VIM plugin.]
